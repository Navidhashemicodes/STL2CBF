...
[JP, J_STL_h, J_STL_s] = expectations_Quadrotor([0,1],[0,2*pi],[-pi/2, pi/2], [-0.01, 0.01], As, B, 10, M, Param, Lam, Bet, T,FF);
...
%         color = '-r.' ;
        plot3([s0(1,1) xx(1,:)],[s0(2,1) xx(2,:)],[s0(3,1) xx(3,:)], 'color', [1,0,0], 'Linewidth', 0.75);
...
% 
figure(7)
V_sum=sum(exp(s(:,len+1+T_F+1:len+1+2*T_F)),2);
for j=len+1+T_F+1:len+1+2*T_F
    V=exp(s(:,j));
    V=V./V_sum;
    plot(V,'Linewidth', 2)
    hold on
end    
V_sum=sum(exp(s(:,len+1+2*T_F+1:len+1+2*T_F+2)),2);
V=exp(s(:,len+1+2*T_F+1));
V=V./V_sum;
