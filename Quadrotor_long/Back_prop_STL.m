function gradient_STL = Back_prop_STL(s0, param, lam_param, beta, M, T,A,B, FF)
...
D_S_step{1}=0;
exp_sum= exp(-eta*CBF1(s{1},FF));
exp_bsum= CBF1(s{1},FF)*exp(-eta*CBF1(s{1},FF));
...
for i=2:T
    ...
    exp_bsum= exp_bsum + CBF1(s{i},FF)*exp(-eta*CBF1(s{i},FF));
...
for i=1:T_F
    ...
    Omega2{i}= (   exp(beta(2*T_F+1))  /  (   exp(beta(2*T_F+1))+exp(beta(2*T_F+2))     )     )*(   exp(beta(i))  /  beta_sum1   )  *   [-2*(s{i}(1:3,1)-center2)'*P2   zeros(1,3)];
end
...
for i=1:T_F
    ...
    Omega3{i}= (   exp(beta(2*T_F+2))  /  (   exp(beta(2*T_F+1))+exp(beta(2*T_F+2))     )    )*(  exp(beta(i+T_F))   /  beta_sum2  )  *   [-2*(s{i}(1:3,1)-center3)'*P3   zeros(1,3)];
end

sq_sum=exp(beta(2*T_F+1))*w_bar1  +  exp(beta(2*T_F+2))*w_bar2;
beta_sum=exp(beta(2*T_F+1))  +  exp(beta(2*T_F+2));
...
for i=1:T
    E1{i}=exp(-eta*CBF1(s{i},FF))/exp_sum;
end
...
for i=1:T_F
    D_J_beta(i)=(exp(-eta*w_bar)/exp_sum)*(  exp(beta(2*T_F+1))  /  (  exp(beta(2*T_F+1))+exp(beta(2*T_F+2))   )   )  *(  (exp(beta(i))*CBF2(s{i},FF)/beta_sum1)  -  (exp(beta(i))*sq_sum1/(beta_sum1^2))  );
end

for i=1:T_F
    D_J_beta(i+T_F)=(exp(-eta*w_bar)/exp_sum)*(   exp(beta(2*T_F+2)) /  (  exp(beta(2*T_F+1))+exp(beta(2*T_F+2)) )  )  *(  (exp(beta(i+T_F))*CBF3(s{i},FF)/beta_sum2)  -  (exp(beta(i+T_F))*sq_sum2/(beta_sum2^2))  );
end

D_J_beta(2*T_F+1)=(exp(-eta*w_bar)/exp_sum)*(  (  exp(beta(2*T_F+1))*w_bar1/beta_sum  )  -  (exp(beta(2*T_F+1))*sq_sum/(beta_sum^2))  );
D_J_beta(2*T_F+2)=(exp(-eta*w_bar)/exp_sum)*(  (  exp(beta(2*T_F+2))*w_bar2/beta_sum  )  -  (exp(beta(2*T_F+2))*sq_sum/(beta_sum^2))  );
