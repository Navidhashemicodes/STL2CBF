function [JP, J_STL_h, J_STL_s] = expectations_Quadrotor(rrs,pphis,tthetas, deltas, A_mean, B, numbers, M, Param, Lam, Bet, T,FF)
...
for ii=1:numbers
    ...
    rob_hard(ii) = robustness_Quadrotor(s0,Param, M, T, A, B,FF);

    [rob_soft(ii), ~] = STL_obj(s0, Param, Lam, Bet, M, T, A, B,FF);
    rewards(ii) = Perf_obj(s0, Param, M, T, A, B,FF);
end
