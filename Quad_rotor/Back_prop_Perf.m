function gradient_Perf = Back_prop_Perf(s0, param, M, T,A,B)
...
Omega{1}=[-2*(s{1}(1:3,1)-dest)'*reward(s{1})/((FF^2)*36)  zeros(1,3)] ;
for i=2:T
    ...
    Omega{i}=[-2*(s{i}(1:3,1)-dest)'*reward(s{i})/((FF^2)*36)   zeros(1,3)];
end
